...
def classify_asymmetry(k_as, metric="rms"):
    """
    Classify asymmetry level based on K_As value.
    
    Args:
        k_as: Asymmetry coefficient value
        metric: "rms" or "skew" - determines which thresholds to use
    
    Returns:
        (class_number, class_name)
    """
    if metric == "skew":
        thresholds = config.ASYMMETRY_THRESHOLDS_SKEW
    else:
        thresholds = config.ASYMMETRY_THRESHOLDS_RMS
    
    if k_as < thresholds["healthy"]:
        return 1, "Healthy"
    elif k_as < thresholds["mild"]:
        return 2, "Mild Asymmetry"
    elif k_as < thresholds["moderate"]:
...
def analyze_recording(data):
    ...
    for ext_ch, flex_ch in pairs:
        ...
        # Classify (using metric-specific thresholds)
        rms_class, rms_label = classify_asymmetry(k_as_rms, metric="rms")
        skew_class, skew_label = classify_asymmetry(k_as_skew, metric="skew")
